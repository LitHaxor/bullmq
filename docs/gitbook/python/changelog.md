...
- **flow-producer:** atomic flow enqueue with strict root-error propagation and per-queue `defaultJobOptions` merging that does not mutate caller dicts [python]. See [Flow Producer](flow-producer.md).
- **job-scheduler:** port `JobScheduler` with `every`/`pattern` repeat strategies, IANA timezones, `startDate`/`endDate`/`limit`/`offset`/`immediately` options, and custom `repeat_strategy` callables [python]. See [Job Scheduler](job-scheduler.md).
- **lock-manager:** extract `LockManager` with batched `extendJobLocks` Lua renewal on a single `asyncio.Task`, idempotent `start()`/`close()`, and introspection helpers [python]. See [Lock Manager](lock-manager.md).
- **queue:** add `setGlobalConcurrency` / `getGlobalConcurrency` / `removeGlobalConcurrency` and `setGlobalRateLimit` / `getGlobalRateLimit` / `removeGlobalRateLimit` backed by the queue `meta` hash [python]. See [Global Concurrency and Rate Limit](global-concurrency-and-rate-limit.md).
- **worker:** cooperative per-job cancellation via an `AbortController` / `AbortSignal` pair; `Worker.cancelJob(job_id)` and `Worker.cancelAllJobs()` flip the signal, and `Worker.close(force=True)` propagates abort then cancels processor tasks [python]. See [Job Cancellation](job-cancellation.md).
- **queue-events:** add `QueueEvents` listener (dedicated XREAD consumer with per-job channel subscriptions and JSON-decoded `progress`/`returnvalue`) and `QueueEventsProducer` for custom events via `XADD MAXLEN ~ maxEvents` [python]. See [Queue Events](queue-events.md).

### Dependencies

- add `croniter ==2.0.7` (required by `JobScheduler`'s `pattern` repeat strategy).
