...
class TestQueueEvents(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_emits_added_event_with_job_id(self):
        ...
        try:
            # Give the consumer a moment to attach to the stream before
            # we publish, otherwise the default lastEventId='$' would
            # skip our event.
            await asyncio.sleep(0.1)

            received: Future = asyncio.get_event_loop().create_future()
    ...
    async def test_per_job_event_channel(self):
        ...
        try:
            ...
            target_done: Future = asyncio.get_event_loop().create_future()
